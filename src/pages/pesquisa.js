...
export default function Pesquisa(){
    ...
    const buscaDadosClientes = useCallback(() => {
        (async () => {
            handleLoad(true)
            try{
                const response = await Api.get(`/negocio/busca?search=${textPesquisa}&categoria=${pesquisaPorCategoria}`);
    ...
    return(
        <>
            ...
            <Header onPesquisa={content => Pesquisar(content)} title='Pesquisa - Disk Aripuanã'/>
            <div className='controle'>
                <div className='titulos-paginas'>
                    Você está em <span>Pesquisa</span>
                </div>
